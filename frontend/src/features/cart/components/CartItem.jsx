...
export const CartItem = ({id, thumbnail, title, category, brand, price, quantity, stockQuantity, productId, size}) => {
        ...
        <Stack bgcolor={'white'} component={is900?'':Paper} p={is900?0:2} elevation={1} flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}>
            ...
            <Stack flexDirection={'row'} rowGap={'1rem'} alignItems={'center'} columnGap={2} flexWrap={'wrap'}>
                ...
                <Stack alignSelf={''}>
                    ...
                    <Typography variant='body2' color={'text.secondary'}>Marimea: {size}</Typography>
                    
                    <Typography mt={1}>Cantitate</Typography>
            ...
            <Stack justifyContent={'space-evenly'} alignSelf={is552?'flex-end':''} height={'100%'} rowGap={'1rem'} alignItems={'flex-end'}>
                <Typography variant='body2'>{price} RON</Typography>
                <Button size={is480?"small":""} onClick={handleProductRemove} variant='contained'>Elimina</Button>
            </Stack>
        </Stack>
    )
}