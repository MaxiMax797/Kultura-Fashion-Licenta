...
import { checkAuth, forgotPassword, login, logout, resendOtp, resetPassword, signup, verifyOtp, skipOtpVerification } from './AuthApi'
...
export const skipOtpVerificationAsync = createAsyncThunk(
    'auth/skipOtpVerificationAsync',
    async(userId) => {
        const res = await skipOtpVerification(userId);
        return res;
    }
);
...
const authSlice=createSlice({
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(skipOtpVerificationAsync.pending, (state) => {
            state.otpVerificationStatus = 'pending';
            })
            .addCase(skipOtpVerificationAsync.fulfilled, (state, action) => {
                state.otpVerificationStatus = 'fulfilled';
                state.loggedInUser = action.payload;
            })
            .addCase(skipOtpVerificationAsync.rejected, (state, action) => {
                state.otpVerificationStatus = 'rejected';
                state.otpVerificationError = action.error;
            })     
    }
