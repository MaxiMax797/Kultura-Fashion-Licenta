...
export const fetchProductsAsync = createAsyncThunk(
    ...
    async (_, { getState }) => {
        const state = getState().ProductSlice;
        const finalFilters = {
            ...state.currentFilters,
            pagination: { page: state.currentPage, limit: ITEMS_PER_PAGE },
            sort: state.currentSort
        };
        
        
        if (state.searchQuery) {
            finalFilters.searchQuery = state.searchQuery;
        }
        ...
        const response = await fetchProducts(finalFilters);
        return response;
    }
);
...
const productSlice=createSlice({
    ...
    reducers:{
        ...
        setCurrentFilters: (state, action) => {
            ...
            state.currentPage = 1; // Resetare la schimbarea filtrelor
        },
        setCurrentSort: (state, action) => {
            state.currentSort = action.payload;
            state.currentPage = 1; // Resetare la schimbarea sortarilor
        },
        setCurrentPage: (state, action) => {
            state.currentPage = action.payload;
        },
        resetAllProductFilters: (state) => { // Buton pentru resetare totala (revenire la stadiu initial)
            state.currentFilters = {};
        ...
        //     state.currentPage = 1; // resetare la pagina 1 atunci cand cauta
        // }

        setSearchQuery: (state, action) => {
            // Resetare doar daca query-ul chiar s-a schimbat
            if (state.searchQuery !== action.payload) {
                state.searchQuery = action.payload;
                state.currentPage = 1; // Resetare pagina la cautare noua
            } else {
                state.searchQuery = action.payload;
                // Nu se va reseta pagina daca returnam aceeasi cautare
            }
        },
    },
...
export const selectCurrentProductFilters = (state) => state.ProductSlice.currentFilters;
export const selectCurrentProductSort = (state) => state.ProductSlice.currentSort;
export const selectCurrentProductPage = (state) => state.ProductSlice.currentPage;
